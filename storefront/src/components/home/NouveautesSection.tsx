...
function getCategoryLabel(product: Product): string {
  ...
  const map: Record<string, string> = {
    "Hauts": "HAUT", "Bas": "BAS", "Vestes & Manteaux": "VESTE",
    "Casquettes": "CASQUETTE", "Lunettes de soleil": "LUNETTES",
    "Cache-cou": "CACHE-COU", "Vêtements": "VÊTEMENT",
    "Accessoires": "ACCESSOIRE", "Chaussures": "CHAUSSURES",
...
// ── Homepage Product Card — Editorial, no quick-add (Stone Island homepage style) ──
...
function ProductCard({ product }: { product: Product }) {
  ...
  // Color bar scroll ref for chevrons
  ...
  useEffect(() => {
    ...
    if (!el) return
    setShowChevrons(el.scrollWidth > el.clientWidth)
  }, [colors])
  ...
  return (
    ...
    >
      ...
      <div className="pt-4 px-[10px] lg:px-[14px]">
        {/* Row 1: Category label (left) + Price (right) */}
        ...
        {/* Row 3: Color bars — Stone Island style (click to select, underline active) */}
        {hasColors && (
          <div className="flex items-center gap-2 mt-2.5">
            ...
            {showChevrons && (
              ...
              >
                <svg width="8" height="12" viewBox="0 0 8 12" fill="none">
                  <path d="M7 1L2 6l5 5" stroke="currentColor" strokeWidth="1" />
                </svg>
              </button>
            )}

            {/* Color bars scrollable */}
            <div
              ref={colorScrollRef}
            ...
            {/* Active color name */}
            <span className="text-[10px] text-muted-foreground tracking-[0.05em] shrink-0 ml-1">
              {activeColor}
            </span>
          </div>
        )}
