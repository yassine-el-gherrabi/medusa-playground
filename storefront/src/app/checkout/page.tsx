...
export default function CheckoutPage() {
    ...
    return (
      <div className="max-w-2xl mx-auto px-4 py-16 text-center animate-fade-in">
        ...
        <p className="text-muted-foreground mb-8">
          Merci pour votre commande. Numero de commande :{" "}
  ...
  const handleAddressSubmit = async (data: { email: string; shipping_address: any }) => {
    setLoading(true)
    setError("")
    try {
      ...
      const msg =
        err?.message || err?.body?.message || "Impossible de sauvegarder l'adresse. Verifiez vos informations."
      setError(msg)
  ...
  const handleShippingSelect = async (optionId: string) => {
    ...
    try {
      ...
      const msg = err?.message || err?.body?.message || "Impossible de definir le mode de livraison."
      setError(msg)
  ...
  const handleOrderComplete = (completedOrder: any) => {
    ...
    <div className="max-w-4xl mx-auto px-4 py-8 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2">
          {step === "address" && <CheckoutForm onSubmit={handleAddressSubmit} loading={loading} />}
          {step === "shipping" && (
            <ShippingOptions
              cartId={cart.id}
          ...
          {step === "payment" && (
            <StripePayment cartId={cart.id} onComplete={handleOrderComplete} />
          )}
        </div>
          ...
          <div className="space-y-3">
            {cart.items?.map((item: any) => (
              <div key={item.id} className="flex justify-between text-sm">
                ...
                <span>
                  {formatPrice(item.unit_price * item.quantity, cart.currency_code || "eur")}
                </span>
              </div>
            ))}
            <div className="border-t border-border pt-3 flex justify-between font-semibold">
              <span>Total</span>
              <span>{formatPrice(cart.total || 0, cart.currency_code || "eur")}</span>
            </div>
          </div>
        </div>
      </div>
